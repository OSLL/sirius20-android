...
class MenuZoneSelect : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        list1.setOnItemClickListener { _, _, position, _ ->
            Log.d("MenuZS","${ZoneArrayFiltered[position]}, ${maskFiltered[position]}")
            if ((maskFiltered[position]>-1) || PreferenceManager.getDefaultSharedPreferences(applicationContext).getBoolean("allowWandering",false))
