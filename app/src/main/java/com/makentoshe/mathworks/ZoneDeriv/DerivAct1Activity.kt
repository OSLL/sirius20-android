    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        continueButtonTask.setOnClickListener {
            ...
            if (i==taskTypes.size) {
                ...
                if ((score.toDouble()/max.toDouble()*100.0).toInt()>=50 && PreferenceManager.getDefaultSharedPreferences(applicationContext).getInt("statusDerivAct2",-1)<0) PreferenceManager.getDefaultSharedPreferences(applicationContext).edit().putInt("statusDerivAct2",0).apply()
                startActivity(intt)
            }
            ...
            if (i<taskTypes.size) {
                ...
                mathview.latex=when(i){
                    1 -> {
                        "\\lim_{x \\to " + values[2] + "}x^" + values[1]
                    }
                    2 -> {
                        "\\lim_{x \\to \\infty}\\frac{" + values[2] + "x+${(1..99).random()}}{" + values[1] + "x-${(1..99).random()}}"
                    }
                    3 -> {
                        formulas[3].toString()
                    }
                    4 -> {
                        "\\lim_{x \\to " + values[1] + "}" + values[2] + "x^2+\\lim_{x \\to " + values[1] + "}" + values[3] + "x"
                    }
                    5 -> {
                        formulas[5].toString()
                    }
...
fun valueMakerForDerivAct1(i: Int): Array<String>{
    ...
    when(i) {
        1 -> {
            num1 = (2..3).random();num2 = (4..10).random();n1 = "$num1";n2 = "$num2"; ans = "${num2.toDouble().pow(num1).toInt()}"
        }
        2 -> {
            num1 = (2..10).random();num2 = (2..10).random() * num1;ans = "${num2 / num1}";n1 = "$num1";n2 = "$num2"
        }
        4 -> {
            num1 = (2..10).random(); num2 = (2..4).random(); num3 = (2..10).random(); n1 = "$num1"; n2 = "$num2"; n3 = "$num3"; ans = "${num1 * num2 * num1 + num1 * num3}"
        }
        5 -> {
            ans = "1; e"
        }
