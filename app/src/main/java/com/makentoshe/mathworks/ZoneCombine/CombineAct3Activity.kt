    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        continueButtonTask.setOnClickListener {
            ...
            if (i==taskTypes.size) {
                ...
                if ((score.toDouble()/max.toDouble()*100.0).toInt()>=50 && PreferenceManager.getDefaultSharedPreferences(applicationContext).getInt("statusCombineAct4",-1)<0) PreferenceManager.getDefaultSharedPreferences(applicationContext).edit().putInt("statusCombineAct4",0).apply()
                startActivity(intt)
            }
